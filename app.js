...
button.addEventListener('click', () => {
    // only run if qbs < 3
    if (qbs < 3) { 
      let ul = document.getElementById('qbList');
...
function attachListItemButton(li) {
  ...
  /*for(let i = 0; i <= li.length; i++) {
    ...
    const rbListItems = document.querySelectorAll('#rbList li');
    const rbList = document.querySelector('#rbList');
console.log(rbList)

//convert ul items to an array
let rbArray = []
for (let i = 0; i < rbListItems.length; i++) {
  rbArray.push(rbListItems[i].textContent)
}
console.log(rbArray);
...
for (let i = 0; i < rbArray.length; i++) {
  ...
  newLi.textContent = rbArray[i]
  rbList.appendChild(newLi)
}
...
function oddsMessage () {
    let odds = document.getElementById('odds')
    odds.textContent = 'SuperBowl Odds';
    //odds.insertAdjacentHTML("afterbegin", `<h2><strong>Vegas SuperBowl Odds</strong></h2>`);

}
oddsMessage();

//h2Heading.appendChild(h2Heading);
//odds = document.body.appendChild(H2);
